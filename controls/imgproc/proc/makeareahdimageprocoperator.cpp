#include "makeareahdimageprocoperator.h"

MakeAreaHdImageProcOperator::MakeAreaHdImageProcOperator(QObject* parent)
...
void MakeAreaHdImageProcOperator::operatorImage(Mat& src, Mat& dest, ImageProcInfo& info)
{
    dest = src(Rect(info.selLeft, info.selTop, info.selWidth, info.selHeight)).clone();
    ...
    if (info.autoThresholdValue)
    {
        threshold(tempData, tempData, 0, 255, THRESH_BINARY | THRESH_OTSU);
    ...
    bitwise_not(tempData, tempData);
    blur(tempData, tempData, Size(3, 3));
