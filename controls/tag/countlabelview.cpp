...
void CountLabelView::paintEvent(QPaintEvent* event)
{
    ...
    QPainter painter;
    painter.begin(this);
#if (QT_VERSIN<QT_VERSION_CHECK(6,0,0))
...
#if (QT_VERSIN<QT_VERSION_CHECK(6,0,0))
        int w = fm.boundingRect(item->caption() + item->value()).width() + 64;
        int w1 = fm.boundingRect(item->caption()).width() + 32;
        int w2 = fm.boundingRect(item->value()).width() + 32;
#else
