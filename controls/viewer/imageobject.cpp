...
bool ImageObject::loadImage(bool isEcrypt)
{
    ...
    if (m_hasLoad == false)
    {
        qDebug() << "loadImage" << QDateTime::currentDateTime();
        if (isEcrypt)
        {
#ifdef USE_AES
        ...
        {
            QString imgPath = thumbnailFilePath(m_path);
            m_pix = QPixmap(imgPath);
            if (m_pix.width() > m_pix.height())
            {
                if (m_pix.width() > 1920)
                {
                    m_pix = m_pix.scaledToWidth(1920);
                }
            }
            else
            {
                if (m_pix.width() > 1080)
                {
                    m_pix = m_pix.scaledToHeight(1080);
                }
            }
        }
