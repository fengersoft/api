...
void CaptureWindow::paintEvent(QPaintEvent* event)
{
    ...
    QScreen* screen = qApp->screens().at(m_screenIndex);
...
int CaptureWindow::screenIndex() const
{
    return m_screenIndex;
}

void CaptureWindow::setScreenIndex(int newScreenIndex)
{
...
void startShootScreen(bool hideWindow)
{
    ...
    for (int i = 0; i < qApp->screens().count(); i++)
    {
        ...
        captureWindow->setScreenIndex(i);
        if (hideWindow)
        {
            activeWindow->show();
        }

        captureWindow->setGeometry(screen->geometry());
        captureWindow->show();
