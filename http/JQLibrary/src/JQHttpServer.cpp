...
{
    //timerForClose_->setInterval(15 * 1000);
    // qDebug() << tcpSocket;
    tcpsocket_ = static_cast<QTcpSocket*>(tcpSocket.data());
...
QMap< QString, QString > JQHttpServer::Session::requestUrlQuery() const
{
    ...
    QString requestStr = requestUrl_.mid(indexForQueryStart + 1);
    requestStr = requestStr.replace(":", "%3A");
    auto lines = QUrl::fromEncoded(requestStr.toUtf8()).toString().split("&");
...
JQHttpServer::AbstractManage::AbstractManage(const int& handleMaxThreadCount)
{
    ...
    handleThreadPool_->setMaxThreadCount(QThread::idealThreadCount());
    handleThreadPool_->setExpiryTimeout(-1);
    serverThreadPool_->setMaxThreadCount(QThread::idealThreadCount());
    serverThreadPool_->setExpiryTimeout(-1);
}
...
{
    if (availableSessions_.contains(session.data()))
    {
        qDebug() << "error";
        return;
    }
    if ((session == nullptr) || (session.data() == nullptr))
    {
        qDebug() << "error session is Null";
        return;
    }
...
bool JQHttpServer::TcpServerManage::onStart()
{
    ...
    QObject::connect(tcpServer_.data(), &QTcpServer::newConnection, [ this ]()
    {
        ...
        connect(socket, &QTcpSocket::disconnected, this, &JQHttpServer::TcpServerManage::clientDisconnected, Qt::BlockingQueuedConnection);

        this->newSession(new Session(socket));
    });
    tcpServer_->setMaxPendingConnections(1024);
...
void JQHttpServer::TcpServerManage::clientDisconnected()
{
    QTcpSocket* tempSocket = static_cast<QTcpSocket*>(sender());
    tempSocket->deleteLater();
    // qDebug() << "tempSocket->deleteLater();";
