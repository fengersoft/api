...
void FnGrid::paintEvent(QPaintEvent* event)
{
    ...
    for (int i = 0; i < gridRows.count(); i++)
    {
        ...
        for (int j = 0; j < rowData->rowCells.count(); j++)
        {
            ...
            QRect textRc(l + cell->left() + cell->extraLeft(), t + cell->extraTop(), cell->width(), rowData->height() + cell->extraHeight());
            cell->setTextRect(textRc);
            qDebug() << textRc << cell->left() << cell->extraLeft();
    ...
    m_scrollBar->setMaxValue(t - m_helper->itemsTop() - height() / 2);
...
void FnGrid::mouseReleaseEvent(QMouseEvent* event)
{
...
void FnGrid::mouseMoveEvent(QMouseEvent* event)
{
...
int FnGrid::gridWidth() const
{
    return m_gridWidth;
}

void FnGrid::setGridWidth(int newGridWidth)
{
