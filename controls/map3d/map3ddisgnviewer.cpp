...
void Map3dDisgnViewer::paintEvent(QPaintEvent* event)
{
    ...
    else if ((m_operation == OP_ADD_WALL_OBJECT) && (m_helper->mousedownFlag()))
    {
        QPoint pt1 = m_helper->mousedownPoint();
        QPoint endPt;
        if (abs(pt1.y() - curPt.y()) <= 10)
        {
            painter.setPen(QColor(0, 255, 0));
            endPt = QPoint(curPt.x(), pt1.y());
        }
        else if (abs(pt1.x() - curPt.x()) <= 10)
        {
            painter.setPen(QColor(0, 255, 0));
            endPt = QPoint(pt1.x(), curPt.y());
        }
        else
        {
            painter.setPen(Qt::black);
...
void Map3dDisgnViewer::mousePressEvent(QMouseEvent* event)
{
    ...
    else if (m_operation == OP_ADD_WALL_OBJECT)
    {
        m_helper->startMouseDown();
    }
...
void Map3dDisgnViewer::mouseReleaseEvent(QMouseEvent* event)
{
    ...
    else if (m_operation == OP_ADD_WALL_OBJECT)
    {
        ...
        {
            pt2 = QPoint(pt2.x(), pt1.y());
            wallType = 1;
        }

        if (abs(pt1.x() - pt2.x()) <= 10)
        {
            pt2 = QPoint(pt1.x(), pt2.y());
            wallType = 2;
        }
        Map3dObject* obj = new Map3dObject(this);
        obj->setObjectType("wall");
        if (wallType == 1)
        {
            obj->points << QPoint(pt1.x(), pt1.y() - 5);
            obj->points << QPoint(pt2.x(), pt2.y() - 5);
            obj->points << QPoint(pt2.x(), pt2.y() + 5);
            obj->points << QPoint(pt1.x(), pt1.y() + 5);
        ...
        {
            obj->points << QPoint(pt1.x() - 5, pt1.y());
            obj->points << QPoint(pt1.x() + 5, pt1.y());
            obj->points << QPoint(pt2.x() + 5, pt2.y());
            obj->points << QPoint(pt2.x() - 5, pt2.y());
