...
void ImageProcViewer::loadFromFile(QString pathName)
{
    ...
    m_imgData = cv::imread(m_pathName.toLocal8Bit().data(), IMREAD_COLOR);
    if (m_imgData.empty())
    {
        return;
    }
...
void ImageProcViewer::mouseReleaseEvent(QMouseEvent* event)
{
    ...
    if (m_procName != "")
    {
        ...
        info.stainWidth = m_stainWidth;
        info.stainHeight = m_stainHeight;
        info.thresholdValue = m_threshold;
        info.autoThresholdValue = m_autoThresholdValue;
        info.gammaValue = m_gammaValue;
        QRect imgRc(0, 0, m_imgData.cols, m_imgData.rows);
        QRect selRc(info.selLeft, info.selTop, info.selWidth, info.selHeight);
        QRect inRc = imgRc.intersected(selRc);
        ...
        if (!inRc.isEmpty())
        {
            info.selLeft = inRc.left();
            info.selTop = inRc.top();
            info.selWidth = inRc.width();
            info.selHeight = inRc.height();
...
int ImageProcViewer::gammaValue() const
{
    return m_gammaValue;
}

void ImageProcViewer::setGammaValue(int newGammaValue)
{
    m_gammaValue = newGammaValue;
}

bool ImageProcViewer::autoThresholdValue() const
{
    return m_autoThresholdValue;
}

void ImageProcViewer::setAutoThresholdValue(bool newAutoThresholdValue)
{
    m_autoThresholdValue = newAutoThresholdValue;
}

int ImageProcViewer::threshold() const
{
    return m_threshold;
}

void ImageProcViewer::setThreshold(int newThreshold)
{
...
int ImageProcViewer::stainHeight() const
{
    return m_stainHeight;
}

void ImageProcViewer::setStainHeight(int newStainHeight)
{
    m_stainHeight = newStainHeight;
}

int ImageProcViewer::stainWidth() const
{
    return m_stainWidth;
}

void ImageProcViewer::setStainWidth(int newStainWidth)
{
    m_stainWidth = newStainWidth;
}
...
void ImageProcViewer::loadShortCutConfig(QString path)
{
    ...
    for (int i = 0; i < jarr.count(); i++)
    {
        ...
        shortCut->setKey(QKeySequence(jobj["shortcut"].toString()));
        shortCut->setProperty("caption", jobj["caption"].toString());
        connect(shortCut, &QShortcut::activated, this,
                &ImageProcViewer::onShortCutActive);
...
void ImageProcViewer::onShortCutActive()
{
    QShortcut* shortcut = static_cast<QShortcut*>(sender());
    emit sendShortCut(shortcut->property("caption").toString());
